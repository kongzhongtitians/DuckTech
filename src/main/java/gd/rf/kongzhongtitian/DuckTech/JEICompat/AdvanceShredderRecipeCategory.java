...
public class AdvanceShredderRecipeCategory implements IRecipeCategory<AdvanceShredderRecipe> {
    public static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(DuckTech.MODID, "advance_shredder");
    ...
    public static final RecipeType<AdvanceShredderRecipe> RECIPE_TYPE =
            new RecipeType<>(UID, AdvanceShredderRecipe.class);

    private final IDrawableBuilder background;
    private final IDrawable icon;
    private final IDrawableBuilder slot;

    public AdvanceShredderRecipeCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.drawableBuilder(TEXTURE, 0, 0, 176, 94);
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK,  new ItemStack(DTBlocks.ADVANCE_SHREDDER.get()));
    ...
    @Override
    public RecipeType<AdvanceShredderRecipe> getRecipeType() {
        return RECIPE_TYPE;
    }
    ...
    @Override
    public void setRecipe(@NotNull IRecipeLayoutBuilder iRecipeLayoutBuilder, AdvanceShredderRecipe shredderRecipe, IFocusGroup iFocusGroup) {
    ...
    @Override
    public void draw(AdvanceShredderRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        IRecipeCategory.super.draw(recipe, recipeSlotsView, guiGraphics, mouseX, mouseY);

        // 绘制文字
        Component text = Component.translatable("block.ducktech.advance_shredder");
